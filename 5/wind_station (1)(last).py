...
#topics
sub_topic = 'wind2'
pub_topic = 'fw'    #from wind


#Serial Start
ser = sr.Serial('/dev/ttyS0', baudrate=9600, timeout=0.2)
...
def on_connect(wind, userdata, flags, rc):
    print('connected')
    wind.subscribe("sub_topic")  # 토픽 구독
...
def send_message(s_sender,s_receiver,s_command,s_data):
    ...
    wind.publish(pub_topic, json_message)
...
while True:
    ...
    if ser_message == 'D\n': #UltraSensor detected
        send_message('wind2','receiver','A',0)
        print('sended')
        t=time.time()
        while True:
            #print(command, 12)
            ...
            if command == 'R':
                ...
                serial_send('R')
            if ser_message=='F\n':
                send_message('wind2','receiver','F',None)
